...
updateWorkspaceFromDBMangedtoVnetInjected() {
  ...
  OrigWSARMtemplate=$(az group export --skip-resource-name-params --resource-group ${globalResourceGroupName} --resource-ids ${globalWorkspaceResourceID} 2>> ${workspaceLogFileNamePrefix}.err)

  log_message Original Arm Template Saved, New Template saved as  ${workspaceLogFileNamePrefix}.json

  echo ${OrigWSARMtemplate} >> ${workspaceLogFileNamePrefix}.log
  echo "--------------------------" >> ${workspaceLogFileNamePrefix}.log
